import site from '../data/site-content.json'

export const ru = {
  meta: {
    title: 'Поступление в вузы Китая | China Dao',
    description:
      'Помогаю поступить в китайские вузы: подбираю программы и стипендии, готовлю документы, подаю заявки и консультирую на каждом этапе.',
  },
  nav: {
    language: 'Язык',
    aria: 'Основная навигация',
    openMenu: 'Открыть меню',
    closeMenu: 'Закрыть меню',
    taglineBefore: 'поступление в ',
    taglineChina: 'Китай',
    links: [
      { label: 'Обо мне', href: '#obo-mne' },
      { label: 'Услуги', href: '#uslugi' },
      { label: 'Этапы работы', href: '#process' },
      { label: 'Цены', href: '#pricing' },
      { label: 'Отзывы', href: '#testimonials' },
    ],
  },
  hero: {
    titleLines: ['Помогу осуществить', 'мечту обучаться', 'в Китае'],
    paragraphs: [
      '你好，我的朋友! Меня зовут Екатерина Гантимурова, и уже более 5 лет я помогаю студентам поступать в лучшие вузы Китая.',
      'Если вы тоже мечтаете учиться в Китае, я с радостью стану вашим личным куратором. Мы вместе пройдём весь путь: от выбора программ обучения и подготовки документов до успешного зачисления в университет.',
      'Обучение в Китае — это уникальная возможность получить качественное международное образование, и я готова сделать путь к поступлению максимально простым и понятным для вас. Доверьтесь моему опыту, и вместе мы достигнем вашей цели!',
    ],
    cta: 'Связаться со мной',
    badge: '+110 успешных поступлений',
    photoAlt: 'Екатерина Гантимурова',
  },
  offers: {
    before: 'Более',
    highlight: '130 офферов',
    after: 'на бюджетное обучение в Китае',
    text: 'Мои клиенты из разных стран уже обладатели более 130 офферов на обучение в Китае с полными стипендиями, и уже сейчас получают высшее образование в Поднебесной.',
    imageAlt: 'Оффер',
  },
  about: {
    title: 'Обо мне',
    cta: 'Подробнее',
    extraTitle: 'Мой опыт и достижения',
    paragraphs: [
      'Привет! Меня зовут Екатерина Гантимурова, и моя цель — помочь вам поступить в один из ведущих университетов Китая. Я сама прошла этот путь: обучалась на языковых курсах и на магистратуре в Shanghai International Studies University по специальности «China Studies», а после в Shanghai Jiao Tong University по направлению «Translation and Interpreting», получив три престижные стипендии — Confucius Institute Scholarship, Chinese Government Scholarship и стипендию от SJTU.',
      'Более 5-ти лет помогаю студентам поступать в Китай. Работала в сфере образования в компании Univibes, где являлась топовым куратором по Китаю. Оказывала помощь в поступлении в учебные заведения Китая, консультируя иностранных студентов, помогая с подбором программ, подготовкой документов и подачей заявок.',
    ],
    sections: [
      { heading: 'Образование' },
      { heading: 'Участие в международных конференциях' },
      { heading: 'Волонтёрский опыт' },
      { heading: 'Опыт работы' },
    ],
    entries: [
      {
        date: '• 2013–2018 гг.',
        place: 'Белорусский государственный университет, Минск, Беларусь',
        texts: [
          'В 2013 году поступила на филологический факультет Белорусского государственного университета в Минске, Беларусь, по специальности «Восточная филология» (Китайская филология).',
          'В 2018 году окончила университет, получила диплом специалиста с квалификацией филолога, преподавателя китайского и немецкого языков, преподавателя китайской литературы, переводчика.',
        ],
      },
      {
        date: '• 2016–2017 гг.',
        place: 'Shanghai International Studies University, Шанхай, Китай',
        texts: [
          'В 2016 году получила Confucius Institute Scholarship (Full Scholarship) и прошла годовую стажировку по китайскому языку в Shanghai International Studies University (SISU), Шанхай, Китай, в рамках программы Chinese Language Exchange Program.',
        ],
      },
      {
        date: '• 2018–2020 гг.',
        place: 'Shanghai International Studies University, Шанхай, Китай',
        texts: [
          'В 2018 получила Chinese Government Scholarship (Full Scholarship) и поступила в магистратуру Shanghai International Studies University (SISU) по специальности «China Studies».',
          'В 2020 году на отлично защитила магистерскую диссертацию и получила степень магистра права по специальности Master of Law in China Studies.',
        ],
      },
      {
        date: '• 2020–2023 гг.',
        place: 'Shanghai Jiao Tong University, Шанхай, Китай',
        texts: [
          'Обучалась, получая университетскую стипендию, на международной магистратуре в Shanghai Jiao Tong University (SJTU) по специальности «Translation and Interpreting».',
        ],
      },
      {
        date: '• 2019 г.',
        place: 'Студенческий Workshop, Kyushu University, Фукуока, Япония',
        texts: [
          'Была выбрана для участия в 4-ом совместном студенческом Workshop между School of International Relations and Public Affairs в Shanghai International Studies University (SISU) и Comparative Studies of Politics and Administration in Asia (CSPA) в Kyushu University. Конференция проходила в городе Фукуока, Япония.',
        ],
      },
      {
        date: '• 2015 г.',
        place: 'Волонтёрство, Оснабрюк, Германия',
        texts: [
          'Участвовала в волонтёрской программе ASF - Aktion Sühnezeichen Friedensdienste в Оснабрюке, Германия.',
          'Работала с международной группой студентов над раскопками исторического здания времён Второй мировой войны. Участие в программе развило навыки межкультурной коммуникации, межличностного взаимодействия, а также углубило знания английского и немецкого языков.',
        ],
      },
      {
        date: '• 2017–2018 гг.',
        place: 'Репетитор русского языка, Минск, Беларусь',
        texts: [
          'Работала репетитором русского языка для китайских студентов, помогая им осваивать язык и развивать языковые навыки для успешной интеграции в образовательную среду.',
        ],
      },
      {
        date: '• 2019–2020 гг.',
        place: 'Учитель английского языка, FINGER FUN Language School, Шанхай, Китай',
        texts: [
          'Работала учителем английского языка в FINGER FUN Language School (Шанхай, Китай). Преподавала детям дошкольного возраста, разрабатывала индивидуальные игровые методики обучения, адаптированные к их культурным и образовательным особенностям.',
        ],
      },
      {
        date: '• 2021–2024 гг.',
        place: 'Куратор по Китаю, компания Univibes, Москва, Россия',
        texts: [
          'Работала куратором по Китаю в компании UNIVIBES, где:',
          '• Консультировала иностранных студентов по вопросам поступления в ведущие университеты Китая.',
          '• Составляла мотивационные и рекомендательные письма, резюме.',
          '• Помогала в подборе подходящих программ, заполнении заявок и подготовке к собеседованиям.',
        ],
      },
    ],
  },
  services: {
    title: 'Услуги, которые я предлагаю',
    tabs: ['Языковые курсы', 'Высшее образование', 'Консультации', 'Подбор программ'],
    items: [
      {
        title: 'Языковые курсы',
        text: 'Полностью сопровождаю процесс поступления на языковые курсы в университеты Китая, включая программы длительностью 1 год (Long-term course) или на полгода (Short-term course). Эти курсы подойдут как для начинающих, так и для тех, кто хочет улучшить свой уровень языка для сдачи экзамена HSK и для успешного поступления на бакалавриат и магистратуру. Я помогу вам выбрать подходящую программу, правильно оформить документы и пройти все этапы поступления.',
      },
      {
        title: 'Высшее образование',
        subsections: [
          {
            subtitle: 'Поступление на бакалавриат',
            text: 'Для поступления на бакалавриат выберем программы, которые соответствуют вашим академическим целям, интересам, карьерным планам и уровню языка. Также подберу подходящие стипендии и помогу на каждом этапе подачи документов для успешного поступления в лучшие университеты Китая.',
          },
          {
            subtitle: 'Поступление на магистратуру',
            text: 'Для поступления на магистратуру я не только подберу подходящие программы и стипендии, но и помогу с подготовкой ключевых документов, включая план диссертации. Также я окажу поддержку в поиске научного руководителя, который соответствует вашим интересам и тематике исследования. Моя задача — сделать этот процесс поступления максимально понятным и комфортным для вас.',
          },
        ],
      },
      {
        title: 'Консультации',
        text: 'Индивидуальная консультация — это основа для успешного выбора образовательной траектории. Перед встречей я изучу вашу анкету, чтобы понять ваши цели, опыт и академические достижения. Во время консультации дам ответы на ваши вопросы. Также мы детально разберем ваши сильные стороны, академический опыт, чтобы оценить ваши шансы на поступление. Обсудим наиболее подходящие программы и возможности, учитывая ваши интересы и перспективы. Вы получите рекомендации, адаптированные под ваш индивидуальный случай, а также четкий план дальнейших шагов для успешного поступления.',
      },
      {
        title: 'Подбор программ',
        text: 'Подбор программ требует тщательного анализа ваших целей, академической подготовки и карьерных планов. Я учитываю все важные детали, чтобы предложить оптимальные варианты обучения, включая программы с возможностью получения стипендии. Вы получите персонализированный список подходящих вариантов, который максимально соответствует вашим требованиям и критериям.',
      },
    ],
  },
  why: {
    title: 'Почему стоит обратиться ко мне?',
    items: [
      { title: 'Индивидуальный подход к каждому студенту', text: 'Я лично сопровождаю вас на всех этапах поступления, вникаю в ваши цели и помогаю раскрыть потенциал. Каждый документ составляется, чтобы подчеркнуть ваши сильные стороны и выделить среди других кандидатов.' },
      { title: 'Многолетний опыт в сфере образования', text: 'Более 5 лет я успешно помогаю студентам поступать в ведущие университеты Китая.' },
      { title: 'Личный опыт обучения и жизни в Китае', text: '3 года проживания в Шанхае по стипендиям, обучение на двух магистратурах и участие в международных конференциях.' },
      { title: 'Глубокое знание системы образования Китая', text: 'Знаю все нюансы поступления, выбора программ и взаимодействия с университетами.' },
      { title: 'Работа с топовыми университетами Китая', text: 'Сотрудничаю только с ведущими государственными вузами, включая стипендиальные программы.' },
      { title: 'Профессиональная коммуникация с вузами', text: 'Свободное владение китайским и английским языками позволяет эффективно взаимодействовать с университетами.' },
    ],
  },
  process: {
    title: 'Процесс работы с клиентом',
    noteTitle: 'Примечание:',
    note: [
      'Во время всех этапов я активно поддерживаю клиента на таком сложном пути, даю четкие инструкции и всегда на связи, чтобы ответить на вопросы и помочь с любыми возникающими трудностями.',
      'Консультирую по вопросам оформления и содержания любых документов, проверяю готовые документы, чтобы они соответствовали требованиям университетов и помогали увеличить шансы на получение стипендии.',
    ],
    steps: site.processSteps,
  },
  pricing: {
    title: 'Цены услуг на полное сопровождение по поступлению на бакалавриат и магистратуру',
    otherTitle: 'Цены на другие услуги',
    noteLabel: 'Примечание:',
    paymentNote: 'Поэтапная оплата в 4-5 платежей',
    duration: '/ до 9 месяцев',
    noteMain: [
      'Подаю на программы, преподаваемые как на китайском, так и на английском языках.',
      { before: 'Количество заявок можно варьировать в зависимости от ваших пожеланий — например, можно выбрать ', strong: '2 или 4 программы', after: '. Мне важно обеспечить вам наилучшие шансы на поступление.' },
    ],
    noteOther: {
      before: 'В качестве исключения также возможна подача на ',
      strong: 'языковые курсы со стипендией Института Конфуция',
      after: '. В этом случае услуга будет считаться как подача на одну или три программы — в зависимости от ваших целей.',
    },
    packages: ['Пакет 1+1', 'Пакет 3+3', 'Пакет 5+5'].map((name) => ({
      name,
      summary: site.pricing[name].summary,
      features: site.pricing[name].features,
    })),
    other: site.otherServices,
  },
  contact: {
    title: 'Оставить заявку:',
    altText: 'Или напишите мне в мессенджере:',
    name: 'Имя',
    messenger: 'Мессенджер',
    nick: 'Ник',
    service: 'Услуга',
    message: 'Сообщение',
    choose: 'Выбрать…',
    submit: 'Оставить заявку',
    sending: 'Отправка...',
    success: 'Заявка отправлена! Я отвечу вам как можно скорее.',
    error: 'Ошибка отправки. Попробуйте ещё раз.',
    serviceOptions: ['Пакет 1+1', 'Пакет 3+3', 'Пакет 5+5', 'Консультация', 'Подбор программ', 'Языковые курсы'],
  },
  testimonials: {
    title: 'Отзывы от моих клиентов',
    readAll: 'Читать все отзывы',
    program: 'Программа',
    degree: 'Степень',
    university: 'Университет',
    specialty: 'Специальность',
    scholarship: 'Стипендия',
    items: site.testimonials,
    extra: site.modalTestimonials,
  },
  faq: {
    title: 'Часто задаваемые вопросы',
    guardian: 'Я не занимаюсь поиском опекуна',
    guardianAlt: 'Я не занимаюсь с поиском опекуна',
    items: site.faq,
  },
  footer: {
    nav: 'Навигация',
    contacts: 'Контакты',
    description:
      'В китайской философии 道 dào означает путь, который отражает естественное течение жизни и гармонию вселенной. В контексте пути к знаниям, дао символизирует стремление к истине, постоянному развитию и самосовершенствованию, подчеркивая важность следования своему внутреннему пути в поисках глубоких знаний и понимания мира.',
    copyright: 'Copyright © 2026. All right reserved to',
  },
}
